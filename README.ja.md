...
image-forge models pull controlnet-canny-sd15  # ControlNet（SD1.5）
image-forge gen -p "a house at night, snow" -m sd15-emaonly \
  --control-net controlnet-canny-sd15 --control photo.png --canny
```

**ControlNet** は **SD1.5** のみ同梱（`controlnet-canny-sd15`）。sd.cpp は original
形式の ControlNet しかロードできず、公開されている SDXL ControlNet は全て diffusers
形式でロード不可のため。`models import <path> --kind controlnet` なら任意の形式を
（描画確認のうえ）登録できる。
