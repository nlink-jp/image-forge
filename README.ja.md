...
| `--seed` | シード（既定 42） |
| `--steps` `--cfg` `-W` `-H` `--sampler` `--clip-skip` | プロファイルを上書き |
...
- **pull**: カタログ名を Hugging Face ソースに解決し、チェックポイントと（カタログ
  エントリなら）専用 VAE を DL してプロファイル登録。生の `hf:owner/repo/file` 参照や
  直 URL も可。
...
`steps`, `cfg`, `width`, `height`, `sampler`, `prediction`, `clip_skip`, `batch`,
`init`, `mask`, `strength`, `loras`（`["path:weight", ...]`）, `output`, `vae`。任意
フィールドが無ければプロファイル既定にフォールバック。
