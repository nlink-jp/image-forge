...
解決する）。ワークスペース内の入力画像も、`.env`、このサーバー自身のディレクトリ内、資格情報ディレクトリ内の
リンクの行き先なら拒否する。いずれも、どんな綴りで渡しても、ファイルがあってもなくても（判定は [nlink-jp/pathguard](https://github.com/nlink-jp/pathguard)
