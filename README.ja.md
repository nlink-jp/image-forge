...
場所、このサーバー自身のデータ・モデルのディレクトリを `work_dir` にすると拒否し、資格情報の場所を生の
パスで指した LoRA・ControlNet・hires のモデルも拒否する —— どんな綴りで渡しても
（判定は [nlink-jp/pathguard](https://github.com/nlink-jp/pathguard) が行う）。
サーバーが触るパスはすべてカーネルがそのワークスペース内に封じ込める。ワークスペース
