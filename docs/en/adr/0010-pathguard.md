...
- `TestExistenceIsNotRevealed` (internal/mcp/tools) calls `generate` and `upscale` with the same path
  ...
  input check's old order and the input floor removed, caught there; the model floor removed and the
  hires model resolved before it is judged, caught in internal/cli) all fail by assertion.
