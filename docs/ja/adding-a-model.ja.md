...
| 項目 | 選び方 |
| --- | --- |
| `Arch` | `ArchSDXL` / `ArchSD15` / `ArchSD35` / `ArchFlux` / `ArchZImage` / `ArchAnima`。Pony・Illustrious は `ArchSDXL`。 |
...
- **LoRA エントリ**は `Kind: catalog.KindLoRA`。アップスケーラと違い **`Arch` 必須** —
  ...
  **LoRA は形式と効果を追加前に検証すること。** sd.cpp は kohya 形式のキー
...
- **`Attribution`** はライセンスがクレジット表記を要求する場合に記載すべき文字列。
  `LicenseFlags` に `LicenseAttribution` を含む**すべての**エントリに設定し、それ以外には
  設定しない（対応関係はテストで強制される）。`"Illustrious XL by ONOMAAI (Civitai)"` の
  ように単体で共有可能なクレジットにすること — ファイルパスは入れない。生成時、レンダリングに
  関与した全モデル（ベースモデル + LoRA）の `Attribution` が重複排除・連結され、出力 PNG の
  `image-forge` メタデータの `credit` フィールドに書き込まれる（画像を共有する人のための記録。
  画素へは一切焼き込まない）。`models list --json` にも `attribution` として公開され、
  フロントエンドが表示・コピーできる。

- **ControlNet エントリ**は `Kind: catalog.KindControlNet`。同じく **`Arch` 必須**。
  `controlnet-canny-sd15`（SD1.5）と `controlnet-canny-sdxl`（SDXL）を同梱。ControlNet
  の変更は **ベースモデルの再ロードを伴う**（エンジンの reload key に含まれる）。レンダ
  ごとに適用される LoRA とはコストが違う点に注意。

  sd.cpp は original 形式（`control_model.` / `input_blocks.` キー。comfyanonymous の
  SD1.5 リパッケージ等）**と diffusers 形式の SDXL ControlNet**（`down_blocks` /
  `controlnet_down_blocks` / `controlnet_cond_embedding` キー）の**両方**をロードできる
  — ロード時に名前を変換し、SDXL の深い transformer 用に ControlNet グラフを確保する
  （vendored sd.cpp の upstream #1752）。よって diffusers 形式の SDXL ControlNet
  （xinsir、lllyasviel/sd_control_collection）は `HF` ソースをそのまま指定でき、事前変換は
  不要。**ただし追加前に必ず描画確認**（`gen --control-net <name> --control <img>
  --canny` で出力が制御エッジに従うこと）— 「描画できていないものは載せない」原則は不変。
- **多コンポーネント**（FLUX / SD3.5 / Z-Image / **Anima**）— `HF`/`Civitai` を空にし、
  ...
  **コンポーネントは Civitai から取得可能:** 各コンポーネント欄（通常は `DiffusionModel`）は
  `civitai:<versionId>` 形式を受け付け、pull 時に Civitai API で解決される（`CIVITAI_TOKEN` 必要）。
  残りは Hugging Face 参照のままにする。これで Civitai ホストの DiT と共有の HF ホスト
  エンコーダ/VAE を組み合わせられる（例: `anima-yume`・`nova-anime-am` = Civitai の Anima DiT +
  `circlestone-labs/Anima` の Qwen3-0.6B + Qwen-Image VAE）。なお Anima の "base" 系は
  `anima-turbo` と違い蒸留されていないため `Steps`/`CFG` の override が必要（CFG1/10 では崩壊、
  CFG5/24 程度で綺麗に出る）。
  **エンコーダは標準 fp8（`t5xxl_fp8_e4m3fn`）・bf16・GGUF のみ使う** — ComfyUI の
  `fp8_scaled` / `fp8_mixed` ビルドは sd.cpp 非互換（ブランク/ロード失敗）。
...
```go
{
    ...
    Attribution:  "Prefect Pony XL by Goofy_Ai (Civitai)", // LicenseAttribution があるので必須
    MinRAMGB:     16, RecRAMGB: 32,
    Source: Source{
...
**`LicenseFlags` / `Attribution` を持つエントリ**は、加えて
[`internal/cli/licenseflags_test.go`](../../internal/cli/licenseflags_test.go) の
ライセンステストも更新する:

- **`TestBaseModelLicenseFlags`** の `want` マップに期待フラグを追加する（未知識別子の
  タイポだけでなく、フラグの取り違え・欠落も捕捉するスポットチェック）。寛容モデルは `nil`。
- 以下は自動で強制されるので個別編集は不要だが、走ることは把握しておく:
  **`TestAttributionFlagMatchesText`**（`attribution` フラグ有なのに `Attribution` テキスト
  が無い／その逆で失敗）、**`TestLicenseFlagsAreKnownAndConsistentWithText`**（未知フラグ、
  または `License` 本文が反映していない `non-commercial` フラグで失敗）。LoRA の
  `TriggerWords` も同様に view テスト経由で表面化する。
...
make vet
./dist/image-forge models list --catalog --json   # エントリ確認（license_flags / attribution / trigger_words 込み）
```
