...
- **ControlNet エントリ**は `Kind: catalog.KindControlNet`。同じく **`Arch` 必須**。
  `controlnet-canny-sd15` を同梱（`sd15-emaonly` で検証済み）。ControlNet の変更は
  **ベースモデルの再ロードを伴う**（エンジンの reload key に含まれる）。レンダごとに
  適用される LoRA とはコストが違う点に注意。

  **sd.cpp は original 形式の ControlNet のみロードできる**（`control_model.` /
  `input_blocks.` キー。comfyanonymous の SD1.5 リパッケージ等）。diffusers→ControlNet
  変換は無いため、**diffusers 形式**（`down_blocks` / `controlnet_down_blocks` /
  `controlnet_cond_embedding` キー）は "failed to load model" で落ちる。**SDXL
  ControlNet を同梱しない理由**がこれ — 公開されている SDXL canny ControlNet
  （xinsir、lllyasviel/sd_control_collection）は全て diffusers 形式のため。追加前に
  safetensors ヘッダを screen する（HTTP `Range` でヘッダだけ取得でき、全 DL 不要）:
  `control_model.` / `input_blocks.` キーであること（`down_blocks` は不可）。そのうえで
  `gen --control-net <name> --control <img> --canny` で描画し、出力が制御エッジに
  従うことを確認する。検証が済むまでは `models import <path> --kind controlnet` で
  ローカルファイルを登録する。
